...
function addService(configDir, service, done) {
  ...
  var npm = spawn('npm', ['install', navService(service), '--save'], {cwd: configDir});
...
function navService(service){
  if(service == "bitcore-wallet-service"){
    return "git://github.com/navcoin/bitcore-wallet-service.git#d7abd19";
  } else if (service == "insight-api") {
    return "git://github.com/navcoin/"+service+".git";
  } else {
    return service;
  }
}
