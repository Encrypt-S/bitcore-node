...
Bitcoin.DEFAULT_RETRY_ATTEMPTS = 120;
...
Bitcoin.prototype._initDefaults = function(options) {
  ...
  this.retryAttempts = Bitcoin.DEFAULT_RETRY_ATTEMPTS;
...
Bitcoin.prototype._loadSpawnConfiguration = function(node) {
  ...
  $.checkArgument(this.options.spawn, 'Please specify "spawn" in navcoind config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in navcoind config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in navcoind config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './navcoin.conf');
...
Bitcoin.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === bitcore.Networks.testnet) {
    networkPath = 'testnet3/navcoin.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/navcoin.conf';
    }
  }
  return networkPath;
};
...
Bitcoin.prototype._spawnChildProcess = function(callback) {
  ...
  self._stopSpawnedBitcoin(function(err) {
    ...
    async.retry({times: self.retryAttempts, interval: self.startRetryInterval}, function(done) {
      if (self.node.stopping) {
        exitShutdown = true;
        return done();
      }
...
Bitcoin.prototype.getDetailedTransaction = function(txid, callback) {
  ...
  function addOutputsToTx(tx, result) {
    ...
    for(var outputIndex = 0; outputIndex < result.vout.length; outputIndex++) {
      ...
      if (out.scriptPubKey && out.scriptPubKey.type === 'cold_staking' && out.scriptPubKey.addresses && out.scriptPubKey.addresses.length === 2) {
        address = out.scriptPubKey.addresses[1];
        type = 'cold_staking';
      }
      tx.outputs.push({
